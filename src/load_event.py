...
def logo_clickable(image_path, target_url):
    ...
    image = open(image_path, 'rb').read()
    # Agrega el evento de clic a la imagen
    clickable_image = f'<a href="{target_url}" target="_self"><img src="data:image/png;base64,{b64encode(image).decode()}" style="width: min(50vw, 200px); aspect-ratio: 2/1; position: absolute !important; z-index:100; margin-top:-62px;"></a>'
    # Muestra la imagen clickeable
    st.markdown(clickable_image, unsafe_allow_html=True)
